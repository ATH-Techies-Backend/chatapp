...
const Chat = () => {
  ...
  useEffect(() => {
    try {
      ...
      const fetchChatInfo = async () => {
        ...
        const response = await fetch(`${API_SOURCE}/chatroom/${pathId}`)
        if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      //const API_SOURCE = "https://chatapp-backend-htbo.onrender.com"
       const API_SOURCE = "http://localhost:9000"
      const response = fetch(`${API_SOURCE}/chatroom/${pathId}`, options)
      setCurrentChat("")
      if (response.ok) {
  ...
  return (
    <div className="container mx-auto">
      ...
      <div className="mx-auto flex items-center justify-between ">
        <Link to="/chatrooms"
          className="w-1/3 no-underline text-white"
